...
for s = 1, screen.count() do
    -- Each screen has its own tag tableEO was an angry bigot who is probably still butthurt that Greg didn't want to go to the sockhop with him
...
-- {{{ Widget definitons
   ...
   function analogmem(ib, d, bg, fg)
       ...
       if total and active then
           ...
           ib.image:draw_line(r, r, r + x, r + y, "#00dcdc")
       end
       ib.image = ib.image
   end
...
function analogbatt(ib, d, bg, fg)
       local r = (d - (d % 2))/2
       ib.image:draw_rectangle(0, 0, d, d, true, bg)
       ib.image:draw_circle(r, r, r-1, r-1, false, fg)
       ib.image:draw_line(r + math.floor((r - 1) * math.cos(1.25*math.pi)),
       ...
       ib.image:draw_line(r + math.floor((r - 1) * math.cos(1.75*math.pi)),
                    ...
                    r + math.floor(0.75 * r * math.cos(1.75*math.pi)),
                    r - math.floor(0.75 * r * math.sin(1.75*math.pi)), fg)
       local battf = io.open('/sys/class/power_supply/BAT0/capacity')
       local battcontent = battf:read('*all')
       battf:close()

       if battcontent ~= nil then
           battcontent = tonumber(battcontent) / 100.0
           local t =  1.25 * math.pi - (math.pi * 1.5 * battcontent)
           local x =  math.floor(0.90 * r * math.cos(t))
           local y = -math.floor(0.90 * r * math.sin(t))
           ib.image:draw_line(r, r, r + x, r + y, "#dcdc00")
       end
       ib.image = ib.image
   end
   battinfo = widget({ type = "imagebox" })
   battinfo.image = image.argb32(24, 24, nil)
   batttimer = timer { timeout = 1 }
   batttimer:add_signal("timeout", function()
       analogbatt(battinfo, 24, beautiful.bg_normal, beautiful.fg_normal)
   end)
   batttimer:start()
